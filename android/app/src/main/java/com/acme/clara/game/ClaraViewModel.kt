...
class ClaraViewModel : ViewModel() {
     ...
     *  paid or not. Otherwise, while sales are live, Hint is a paid-tier perk (advertised on the
     *  purchase card as "a hint"): an unpaid tap offers the purchase instead of any hint text, and
     *  a paid career gets exactly one concrete Bureau tip per case — a second ask is told there's
     *  nothing left to give, rather than repeating or vaguing out the same lead. While sales are
     *  disabled for this release ([com.acme.clara.billing.BillingManager.SALES_ENABLED] false),
     *  Hint stays exactly as it always was: unlimited, and free hints still cost the hint-free
     *  badge — nothing here should change what today's players already have. */
    fun requestHint() {
        val i18n = com.acme.clara.i18n.Strings
        if (!com.acme.clara.billing.BillingManager.SALES_ENABLED || s.freeHints > 0) {
            val hint = hintText()
        ...
        s = s.copy(hintsUsed = s.hintsUsed + 1, bureauTipUsed = true,
            overlay = Overlay.Info(i18n.ui("HINT"), listOf(hintText())))
        autosave()
    }
